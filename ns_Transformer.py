...
class NS_Transformer(nn.Module):
    ...
    def __init__(self, configs):
        ...
        self.seq_len = configs.seq_in_len
        self.label_len = configs.label_len
        self.output_attention = configs.output_attention

        # Embedding
        self.enc_embedding = DataEmbedding(configs.num_nodes, configs.hidden_size, configs.dropout)
        self.dec_embedding = DataEmbedding(configs.num_nodes, configs.hidden_size, configs.dropout)
        # Encoder
        ...
        self.decoder = Decoder(
            [
                DecoderLayer(
                    AttentionLayer(
                        ...
                        configs.hidden_size, configs.n_heads),
                    AttentionLayer(
            ...
            norm_layer=torch.nn.LayerNorm(configs.hidden_size),
            projection=nn.Linear(configs.hidden_size, configs.num_nodes, bias=True)
        )

        self.tau_learner   = Projector(enc_in=configs.num_nodes, seq_len=configs.seq_in_len, hidden_dims=configs.p_hidden_dims, hidden_layers=configs.p_hidden_layers, output_dim=1)
        self.delta_learner = Projector(enc_in=configs.num_nodes, seq_len=configs.seq_in_len, hidden_dims=configs.p_hidden_dims, hidden_layers=configs.p_hidden_layers, output_dim=configs.seq_in_len)
